			...
			<div className="grid gap-8 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:items-end">
				...
				>
					<div className="flex flex-wrap items-center gap-3">
						...
						<span
							...
							style={{ fontFamily: SANS_STACK }}
						>
							{isSpine ? "Build spine" : "Route atlas"}
						</span>
					</div>
					...
					<div className="grid gap-4 md:grid-cols-3">
						<div className="rounded-[1.7rem] border border-accent-base/15 bg-surface/90 p-5 shadow-[0_18px_52px_-30px_rgba(24,24,27,0.28)]">
							...
							<p
								...
								style={{ fontFamily: SERIF_STACK }}
							>
								{isSpine ? "Choose the next build stage" : "Match the page to the job"}
							</p>
						</div>
						<div className="rounded-[1.7rem] border border-accent-base/15 bg-surface/90 p-5 shadow-[0_18px_52px_-30px_rgba(24,24,27,0.28)]">
							<p
							...
							<p
								...
								style={{ fontFamily: SERIF_STACK }}
							>
								{isSpine ? "Keep the artifact chain intact" : "Read in maintained order"}
							</p>
						</div>
						<div className="rounded-[1.7rem] border border-accent-base/15 bg-[#16171a] p-5 text-white shadow-[0_18px_52px_-30px_rgba(24,24,27,0.42)]">
							...
							<p className="mt-3 text-lg leading-tight" style={{ fontFamily: SERIF_STACK }}>
								{isSpine
									? "Every stage ends in something you can run or validate"
									: "Every route points back to a live surface"}
							</p>
						</div>
					</div>
				...
				>
					...
					<figcaption className="space-y-2 border-t border-accent-base/10 bg-surface-lighter/80 px-6 py-5">
						<p className="text-sm font-semibold text-foreground" style={{ fontFamily: SANS_STACK }}>
							{content.heroFigure.title}
						</p>
						<p className="text-sm leading-6 text-color-500" style={{ fontFamily: SANS_STACK }}>
			...
			>
				{content.decisionPanels.map((panel, index) => (
					...
					>
						...
						<p className="mt-3 text-sm leading-7 text-color-500" style={{ fontFamily: SANS_STACK }}>
							{panel.description}
						</p>
						<div
...
function RouteAtlas({ content }: { content: HubPageContent }) {
	if (content.presentationStyle === "spine") {
		return (
			<section id="route-atlas" className="mx-auto max-w-7xl scroll-mt-28 px-4 sm:px-6 lg:px-8">
				<div className="overflow-hidden rounded-[2.25rem] border border-accent-base/15 bg-surface/92 shadow-[0_24px_80px_-34px_rgba(24,24,27,0.28)]">
					<div className="border-b border-accent-base/10 px-6 py-8">
						<p
							...
							style={{ fontFamily: SANS_STACK }}
						>
							Maintained build spine
						</p>
						<h2
							className="mt-4 text-4xl leading-tight text-foreground"
							style={{ fontFamily: SERIF_STACK }}
						>
							Build in sequence so each page hands a real artifact to the next.
						</h2>
						<p
							className="mt-4 max-w-3xl text-sm leading-7 text-color-500"
							style={{ fontFamily: SANS_STACK }}
						>
							Tutorials should feel like a staged runbook. The spine below is a build order, not a
							neutral list of pages.
						</p>
					</div>
								...
								>
									<div className="flex items-center justify-center border-b border-accent-base/10 bg-[#16171a] px-5 py-6 text-white md:border-b-0 md:border-r">
										<div className="text-center">
											...
											<p className="mt-2 text-4xl leading-none" style={{ fontFamily: SERIF_STACK }}>
												{index + 1}
											</p>
										</div>
									</div>
			...
			<div className="overflow-hidden rounded-[2.25rem] border border-accent-base/15 bg-[#16171a] text-white shadow-[0_24px_80px_-34px_rgba(24,24,27,0.45)]">
				<div className="grid gap-0 lg:grid-cols-[0.34fr_0.66fr]">
					<div className="border-b border-white/8 px-6 py-8 lg:border-b-0 lg:border-r">
						...
						<h2 className="mt-4 text-4xl leading-tight" style={{ fontFamily: SERIF_STACK }}>
							Follow the route in order when you want the shortest maintained path.
						</h2>
						<p className="mt-4 text-sm leading-7 text-white/72" style={{ fontFamily: SANS_STACK }}>
							The atlas below is the stable reading and building order. Every stage says what it
							teaches, what surface backs it, and what you should walk away able to do.
						</p>
					</div>
	...
	return (
		...
		>
			...
			<div className="space-y-3">
				<h3 className="text-2xl leading-tight text-foreground" style={{ fontFamily: SERIF_STACK }}>
					{card.title}
				</h3>
				<p className="text-sm leading-7 text-color-500" style={{ fontFamily: SANS_STACK }}>
					{card.description}
				</p>
			</div>
				...
				<div className="grid gap-3 rounded-[1.5rem] border border-accent-base/10 bg-surface-lighter/75 p-4">
					{card.detailRows.map((row) => (
						...
						>
							...
							<span className="text-sm leading-6 text-color-500" style={{ fontFamily: SANS_STACK }}>
								{row.value}
							</span>
						</div>
					))}
...
function HubLane({
	...
	return (
		<section id={section.id} className="mx-auto max-w-7xl scroll-mt-28 px-4 sm:px-6 lg:px-8">
				...
				<div className={`${index % 2 === 1 ? "lg:order-1" : ""}`}>
					<div className="grid gap-5 md:grid-cols-2">
						{section.cards.map((card, cardIndex) => (
							<HubCardTile key={card.id} card={card} iconIndex={index + cardIndex} />
						))}
					</div>
				</div>
			</div>
			...
			>
				<div className="grid gap-0 lg:grid-cols-[0.42fr_0.58fr]">
					...
					>
						...
						>
							{isSpine
								? "Tutorial pages should tell the reader what to run, what to inspect, and what the next stage depends on. If one of those is vague, the route still needs work."
								: "These hubs are supposed to reduce ambiguity. If a route does not make its artifact, proof state, and source surface clear, it still needs work."}
						</p>
					</div>
					...
					<div className="grid gap-4 p-6 md:grid-cols-2">
						{content.actions.map((action) => (
							...
							>
								...
								<div className="mt-5">
									<ActionButton href={action.href} label="Open route" variant={action.variant} />
								</div>
							</div>
						))}
					</div>
