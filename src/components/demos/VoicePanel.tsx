...
const DEFAULT_SCRIPT = "With Vizij your avatar mirrors every beat of the conversation.";
...
export function VoicePanel({
	...
	const visemeBasePath = useMemo(() => `rig/${(faceId ?? "face").toLowerCase()}/visemes`, [faceId]);
	...
	const applyVisemeAt = useCallback((timeMs: number): VisemeTimelineEntry | null => {
		const timeline = visemeTimelineRef.current;
	...
	const hydrateSpeech = useCallback(
		...
		[setSpokenAudio, setSpokenSentences, setSpokenVisemes, setSpokenWords, updateTimeline],
	);
	...
	const handleGenerate = useCallback(async () => {
		...
		try {
			const { visemeData, audioBlob } = await getTTSData(trimmed, selectedVoice);
	...
	return (
		...
		>
			...
			<h3 className="mt-2 text-base font-semibold">Generate a sample line powered by Polly.</h3>
			<p className="mt-2 text-sm text-color-500">
				Request Amazon Polly audio + viseme metadata, then stream visemes into Vizij via runtime
				staging inputs.
			</p>
			...
			{spokenAudio && (
				<div className="mt-3 rounded-md border border-accent-base/20 bg-surface p-2">
					...
					<audio
						...
						onTimeUpdate={(event) => handleAudioScrub(event.currentTarget.currentTime)}
						onSeeked={(event) => handleAudioScrub(event.currentTarget.currentTime)}
						className="w-full"
					/>
				</div>
			)}
...
const createVisemeTimeline = (
	...
	entries.sort((a, b) => a.start - b.start || a.sourceCode.localeCompare(b.sourceCode));
	...
	for (let i = 0; i < entries.length; i += 1) {
		...
		if (next) {
			current.end = next.start > current.start ? next.start : current.start + MIN_VISEME_SPAN_MS;
		} else {
...
const findVisemeIndex = (timeline: VisemeTimelineEntry[], timeMs: number): number => {
