...
export function VoicePanel({
  ...
  const poseGroups = (poseConfig as { poseGroups?: PoseGroupDefinition[] } | null)
    ?.poseGroups;
  const poseWeightPaths = useMemo(
    ...
    [faceId, poseConfig?.faceId, poseConfig?.poses, poseGroups],
  );
  ...
  const clearVisemeInputs = useCallback(() => {
    ...
    for (const path of visemePathsRef.current) {
  ...
  return (
    ...
    >
      ...
      <form className="mt-4 space-y-3" onSubmit={(e) => e.preventDefault()}>
        <label
          className="block text-xs font-semibold uppercase tracking-wider text-accent-base/80"
          htmlFor="voice-script"
        >
          Script
          <textarea
            id="voice-script"
            className="mt-2 min-h-24 w-full rounded-md border border-[#ead7c3] bg-white/84 px-3 py-2 text-sm text-foreground outline-none transition-colors focus:border-accent-base/50 dark:border-white/10 dark:bg-[#0f1318]"
            value={script}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          type="button"
          className="rounded-md border border-accent-base/40 bg-accent-base/10 px-3 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent-base/20 disabled:cursor-not-allowed disabled:opacity-50 dark:border-accent-base/25 dark:bg-accent-base/15 dark:hover:bg-accent-base/22"
        ...
        {status === "speaking" && (
          <button
            type="button"
            className="rounded-md border border-[#ead7c3] bg-white/84 px-3 py-2 text-sm text-foreground transition-colors hover:border-accent-base/45 hover:bg-accent-base/10 dark:border-white/10 dark:bg-[#0f1318] dark:hover:bg-white/6"
      ...
      <div className="mt-3 text-xs text-color-500 dark:text-[#c8c0b4]">
        Status: <span className="text-foreground">{statusCopy.label}</span>
      </div>

      {spokenAudio && (
        <div className="mt-3 rounded-md border border-[#ead7c3] bg-white/84 p-2 dark:border-white/10 dark:bg-[#0f1318]">
          {/* biome-ignore lint/a11y/useMediaCaption: TTS preview audio, captions not applicable */}
          <audio
            key={spokenAudio}
            controls
            src={spokenAudio}
        ...
        <div className="mt-4 space-y-3">
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {spokenWords.map((word, index) => (
                  <span
                    ...
                    className="rounded border border-[#ead7c3] bg-white/84 px-2 py-1 text-xs text-color-500 dark:border-white/10 dark:bg-[#0f1318] dark:text-[#c8c0b4]"
                  >
                    {word.value}
                  </span>
                ))}
          ...
          {faceVisemeLabels.length > 0 && (
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {Array.from(faceVisemeLabels.entries(), ([i, label]) => (
                  <span
                    key={`${label}-${i}`}
                    className={
                      i === activeVisemeIndex
                        ? "rounded border border-accent-base/40 bg-accent-base/15 px-2 py-1 text-xs text-foreground dark:border-accent-base/25 dark:bg-accent-base/18"
                        : "rounded border border-[#ead7c3] bg-white/84 px-2 py-1 text-xs text-color-500 dark:border-white/10 dark:bg-[#0f1318] dark:text-[#c8c0b4]"
                    }
                  >
                    {label}
                  </span>
      ...
      <ul className="mt-4 space-y-1 text-xs text-color-500 dark:text-[#c8c0b4]">
