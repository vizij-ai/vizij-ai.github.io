import { type RuntimeDebugStatus, addRuntimeStatusListener } from "@/demo-lib/runtimeDebug";
	...
	useEffect(() => {
		const id = window.setInterval(() => {
			...
			setRows((prev) => {
				...
				for (const [ns, row] of Object.entries(prev)) {
			...
			{visible && (
				...
				>
					...
					{sorted.map((row) => {
						...
						const fps = row.stepHz && Number.isFinite(row.stepHz) ? row.stepHz.toFixed(1) : "—";
						return (
							<div
								key={row.namespace}
								style={{
									...
									marginBottom: 4,
									borderRadius: 6,
									background: row.stale ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.08)",
									border: row.driver
