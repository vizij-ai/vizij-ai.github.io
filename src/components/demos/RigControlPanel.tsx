...
function countWithConstraints(options: RigInputOption[]): {
	...
	const resolved = options.filter(
		(opt) => opt.min !== undefined || opt.max !== undefined || opt.defaultValue !== undefined,
	).length;
...
function InputStager({ inputs, disabled, onStage, constraintsCount, unstyled }: InputStagerProps) {
		...
		(path: string) => {
			...
			if (
				...
				if (!warnedMissingRef.current.has(path)) {
					...
					console.warn("[vizij-demos] No constraints for path; using fallback range", {
						path,
						min,
						max,
						defaultValue,
	...
	const addPath = useCallback(
		(path: string) => {
			...
			const clamped = existing != null ? existing : Math.min(max, Math.max(min, defaultValue));
			if (existing == null) {
	...
	const filteredInputs = useMemo(() => {
		...
		return inputs.filter((option) => option.path.toLowerCase().includes(term)).slice(0, 20);
	}, [inputs, query]);

	const selectedLabel =
		inputs.find((option) => option.path === selectedPath)?.label ?? selectedPath;
							...
							>
								...
								<div className="mt-2 flex items-center gap-3">
									<input
										...
										step={0.01}
										value={value}
										onChange={(event) => stageValue(path, Number(event.target.value))}
										disabled={disabled}
									/>
									<button
										type="button"
