...
import { type PoseHotkeyBinding, getPoseGroup, usePoseHotkeys } from "@/demo-lib/usePoseHotkeys";
...
export function HeroPassiveBehavior({ enabled = true }: { enabled?: boolean }) {
	...
	const emotionBindings = useMemo(() => filterBindingsByKind(bindings, "emotion"), [bindings]);
	const vizemeBindings = useMemo(() => filterBindingsByKind(bindings, "viseme"), [bindings]);
...
function filterBindingsByKind(bindings: PoseHotkeyBinding[], kind: PoseKind) {
	...
	return bindings.filter((binding) => {
		...
		const target = [getPoseGroup(binding.pose), binding.pose.name, binding.pose.id, ...valueKeys]
			.filter(Boolean)
...
function useAlternatingEmotionVizemeRoutine({
	...
	useEffect(() => {
		...
		const animateBinding = (binding: PoseHotkeyBinding, value: number, duration: number) => {
...
function pickRandomBinding(bindings: PoseHotkeyBinding[], exclude: PoseHotkeyBinding | null) {
