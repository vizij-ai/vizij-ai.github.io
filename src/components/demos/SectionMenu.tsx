...
export function SectionMenu({ sections }: SectionMenuProps) {
	...
	return (
		<>
			{isOpen ? (
				// biome-ignore lint/a11y/useKeyWithClickEvents: backdrop role=presentation closes modal on click, keyboard interaction handled by modal close button
				<div
						...
						{sections.map((section, index) => {
							...
							return (
								<li key={section.id} className="section-menu__item">
									<button
										type="button"
										className={`section-menu__link${isActive ? " is-active" : ""}`}
									...
									>
										<span className="section-menu__link-left">
											...
											<span className="section-menu__name">{section.label}</span>
										</span>
										<span className="section-menu__pill">{isActive ? "Now" : "Go"}</span>
									</button>
								</li>
							);
						})}
