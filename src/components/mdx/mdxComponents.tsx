...
export const mdxComponents: MDXComponentMap = {
  h1: (props: ElementProps<"h1">) => (
    <h1
      ...
      className={clsx(
        headingBase,
        "text-3xl md:text-4xl mt-12 mb-4",
        props.className,
      )}
    />
  ),
  h2: (props: ElementProps<"h2">) => (
    <h2
      ...
      className={clsx(
        headingBase,
        "text-2xl md:text-3xl mt-10 mb-4",
        props.className,
      )}
    />
  ),
  ...
  p: (props: ElementProps<"p">) => (
    <p
      ...
      className={clsx(
        bodyText,
        "my-4 dark:text-neutral-200 text-neutral-800",
        props.className,
      )}
    />
  ),
  a: (props: ElementProps<"a">) => (
    <a
      {...props}
      href={props.href ? resolveSitePath(props.href) : props.href}
      className={clsx(
  ...
  code: (props: CodeElementProps) =>
    isBlockCode(props) ? (
      <code
        {...props}
        className={clsx("font-mono text-[0.95em]", props.className)}
      />
    ) : (
      <code
        {...props}
  ...
  hr: (props: ElementProps<"hr">) => (
    <hr
      {...props}
      className={clsx("my-10 border-t border-accent-one/30", props.className)}
    />
  ),
  ...
  td: (props: ElementProps<"td">) => (
    <td
      {...props}
      className={clsx(
        "px-3 py-2 border border-accent-one/10 align-top",
        props.className,
      )}
    />
  ),
  img: ({ alt = "", src, ...props }: ElementProps<"img">) => (
    <img
      {...props}
      alt={alt}
      src={typeof src === "string" ? resolveSitePath(src) : src}
      className={clsx(
        "my-6 rounded-xl shadow-sm border border-accent-one/10",
        props.className,
      )}
    />
  ),
