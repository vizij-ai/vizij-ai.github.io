...
export async function filterResearch(criteria: {
	...
	if (criteria.yearFrom !== undefined) {
		entries = entries.filter((entry) => {
			const year = getPublishYear(entry);
			// biome-ignore lint/style/noNonNullAssertion: yearFrom is checked via criteria.yearFrom !== undefined guard above
	...
	if (criteria.yearTo !== undefined) {
		entries = entries.filter((entry) => {
			const year = getPublishYear(entry);
			// biome-ignore lint/style/noNonNullAssertion: yearTo is checked via criteria.yearTo !== undefined guard above
