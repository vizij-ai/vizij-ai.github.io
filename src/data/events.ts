...
export async function filterEvents(criteria: {
	...
	if (criteria.dateFrom) {
		// biome-ignore lint/style/noNonNullAssertion: dateFrom is checked via criteria.dateFrom guard above
		events = events.filter((event) => event.data.startDate >= criteria.dateFrom!);
	}

	if (criteria.dateTo) {
		// biome-ignore lint/style/noNonNullAssertion: dateTo is checked via criteria.dateTo guard above
