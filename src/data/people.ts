...
export async function getPerson(id: string): Promise<CollectionEntry<"people"> | undefined> {
...
export async function getPeopleByOrganization(
	...
	return allPeople.filter((person) =>
		person.data.affiliations?.some(
			(aff) => aff.organizationId === organizationId && (!aff.endDate || aff.endDate >= now),
		),
	);
}
...
export async function getPeopleByExpertise(
	...
	const allPeople = await getAllPeople();
	return allPeople.filter((person) =>
		person.data.expertise?.some((exp) => exp.toLowerCase().includes(expertise.toLowerCase())),
	);
}
...
export async function getUniqueExpertiseAreas(): Promise<string[]> {
	const allPeople = await getAllPeople();
	const expertiseAreas = allPeople.flatMap((person) => person.data.expertise || []);
	return [...new Set(expertiseAreas)].sort();
}

/** Get people count by organization */
export async function getPeopleCountByOrganization(): Promise<Record<string, number>> {
	const allPeople = await getAllPeople();
	const counts: Record<string, number> = {};
	const now = new Date();

	for (const person of allPeople) {
		for (const aff of person.data.affiliations ?? []) {
...
export async function searchPeople(query: string): Promise<CollectionEntry<"people">[]> {
	const allPeople = await getAllPeople();
	...
	return allPeople.filter((person) => {
		...
		return (
			...
			person.data.bio?.toLowerCase().includes(lowerQuery) ||
			person.data.title?.toLowerCase().includes(lowerQuery) ||
			person.data.expertise?.some((exp) => exp.toLowerCase().includes(lowerQuery)) ||
...
export async function getFormattedAuthors(
	...
	if (authorNames.length <= maxDisplay) {
		...
		return last ? `${authorNames.join(", ")}, and ${last}` : authorNames.join(", ");
	}
...
export async function getRelatedPeople(
	...
	const scoredPeople = otherPeople.map((person) => {
		...
		if (currentPerson.data.expertise && person.data.expertise) {
			const currentExpertise = new Set(currentPerson.data.expertise.map((e) => e.toLowerCase()));
		...
		if (currentPerson.data.affiliations && person.data.affiliations) {
			...
			const sharedOrgs = person.data.affiliations.filter(
				(a) => (!a.endDate || a.endDate >= now) && currentOrgs.has(a.organizationId),
			);
