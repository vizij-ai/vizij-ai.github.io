...
export async function getSoftwareByLanguage(
	...
	return software.filter((item) =>
		item.data.language.some((lang) => lang.toLowerCase() === language.toLowerCase()),
	);
}
...
export async function getSoftwareByPlatform(
	...
	const software = await getAllSoftware();
	return software.filter((item) =>
		item.data.platform.some((plat) => plat.toLowerCase() === platform.toLowerCase()),
	);
}
...
export async function getSoftwareByLicense(
	...
	const software = await getAllSoftware();
	return software.filter((item) => item.data.license.toLowerCase().includes(license.toLowerCase()));
}
...
export async function getSoftwareByOrganization(
	...
	return software.filter((item) => {
		return (item.data.contributors ?? []).some(
			(contributor) =>
				contributor.type === "organization" && contributor.organizationId === organizationId,
		);
	});
}
...
export async function getSoftwareCountByLanguage(): Promise<Record<string, number>> {
	...
	for (const item of software) {
		for (const lang of item.data.language) {
...
export async function searchSoftware(query: string): Promise<CollectionEntry<"software">[]> {
	...
	return software.filter((item) => {
		const contributorMatch = (item.data.contributors ?? []).some((contributor) => {
		...
		return (
			...
			(item.data.topics ?? []).some((topic) => topic.toLowerCase().includes(lowerQuery)) ||
			contributorMatch
		);
	});
}
...
export async function getRecentlyUpdatedSoftware(
	...
	return software
		.sort((a, b) => {
			const aDate = a.data.publishDate ? new Date(a.data.publishDate) : new Date(0);
			const bDate = b.data.publishDate ? new Date(b.data.publishDate) : new Date(0);
...
export async function filterSoftware(criteria: {
	...
	if (criteria.language) {
		software = software.filter((item) =>
			item.data.language.some((lang) => lang.toLowerCase() === criteria.language?.toLowerCase()),
		);
	}

	if (criteria.platform) {
		software = software.filter((item) =>
			item.data.platform.some((plat) => plat.toLowerCase() === criteria.platform?.toLowerCase()),
		);
	}

	if (criteria.license) {
		const license = criteria.license;
		software = software.filter((item) =>
	...
	if (topicFilters && topicFilters.length > 0) {
		software = software.filter((item) =>
			topicFilters.some((topic) =>
				(item.data.topics ?? []).map((value) => value.toLowerCase()).includes(topic.toLowerCase()),
			),
		);
	}
