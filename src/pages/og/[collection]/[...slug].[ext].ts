...
async function loadOgImageAsset(
	...
	if (typeof meta === "string") {
		...
		if (originalFile) {
			...
			if (candidates?.length && candidates.length > 0) {
				...
				// biome-ignore lint/style/noNonNullAssertion: candidate is guaranteed non-null — set to candidates[0] above and only reassigned to a non-null match
				let cached = assetDataCache.get(candidate!);
				if (!cached) {
					// biome-ignore lint/style/noNonNullAssertion: candidate is guaranteed non-null (see above)
				...
				// biome-ignore lint/style/noNonNullAssertion: cached is assigned raster.src above, so non-null here
				return { src: cached! };
			}
		}
		return undefined;
	...
	// biome-ignore lint/style/noNonNullAssertion: dataUrl is assigned raster.src above, so non-null here
