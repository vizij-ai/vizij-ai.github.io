...
type PoseDefinitionWithMembership = PoseDefinition & {
  groupId?: string | null;
  groupIds?: string[];
};

type PoseMembershipLike = Pick<
  PoseDefinitionWithMembership,
  "group" | "groupId" | "groupIds"
>;

type PoseSemanticLike = Pick<
  PoseDefinitionWithMembership,
  "id" | "name" | "group" | "groupId" | "groupIds"
>;
...
export function filterPosesBySemanticKind(
  ...
  return poses.filter(
    (pose) =>
      resolvePoseSemantics(pose as PoseDefinitionWithMembership, groups).kind ===
      kind,
  );
}
...
export function buildSemanticPoseWeightPathMap(
  ...
  poses.forEach((pose) => {
    const semantics = resolvePoseSemantics(
      pose as PoseDefinitionWithMembership,
      groups,
    );
...
export function getPoseGroup(
  pose: PoseSemanticLike,
): string | undefined {
  const explicitGroup = pose.group?.trim();
  if (explicitGroup) {
    return explicitGroup;
  }

  const membership = resolvePoseMembership(pose, undefined);
  if (membership.primaryGroupPath) {
    return membership.primaryGroupPath;
  }
  if (membership.primaryGroupId) {
    return membership.primaryGroupId;
  }

  const semantics = resolvePoseSemantics(pose, undefined);
  if (semantics.kind === "emotion") {
    return "emotion";
  }
  if (semantics.kind === "viseme") {
    return "viseme";
  }
  return undefined;
}
