...
export function addRuntimeStatusListener(handler: (status: RuntimeDebugStatus) => void) {
	if (typeof window === "undefined") return () => {};
	...
	return () => window.removeEventListener(RUNTIME_STATUS_EVENT, listener as EventListener);
}
