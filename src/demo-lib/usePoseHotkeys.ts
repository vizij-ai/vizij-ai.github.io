import { type PoseDefinition, type PoseRigConfig, useVizijRuntime } from "@vizij/runtime-react";
...
export function usePoseHotkeys(poseConfig: PoseRigConfig | null, enabled: boolean) {
	...
	const bindings = useMemo<PoseHotkeyBinding[]>(() => {
		...
		return poses.map((pose) => {
			...
			const relativeSegment = sanitizePoseSegment(pose.name ?? pose.id, pose.id);
...
function sanitizePoseSegment(value: string | null | undefined, fallback: string): string {
...
function buildPoseWeightPathMap(poses: PoseDefinition[], faceSegment: string): Map<string, string> {
