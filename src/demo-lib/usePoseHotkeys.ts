...
export { getPoseGroup } from "@/demo-lib/posePaths";

const MAX_POSE_WEIGHT = 1;

function clampPoseWeight(weight: number) {
  return Math.min(MAX_POSE_WEIGHT, Math.max(0, weight));
}
...
export function usePoseHotkeys(
  ...
  const bindings = useMemo<PoseHotkeyBinding[]>(() => {
    ...
    return poses.map((pose) => {
      ...
      const semantics = resolvePoseSemantics(pose, poseGroups);
      return {
        pose,
  ...
  }, [poseConfig, poseFaceSegment, poseGroups, poseWeightPaths]);
