...
export function mapPollyViseme(value: string | undefined | null): ResolvedFaceViseme | null {
	...
	if (!mapping) {
		if (!reportedUnknown.has(trimmed)) {
			console.warn(`[voice] Unmapped Polly viseme code: ${trimmed}. Falling back to neutral.`);
	...
	const label =
		segment != null ? (FACE_VISEME_LABELS[segment] ?? segment) : isSilence ? "rest" : trimmed;
	return {
		segment,
		label,
		isSilence,
